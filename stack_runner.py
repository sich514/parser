...
def build_specs(
    ...
    if enable_bybit:
        specs.insert(2, ProcSpec("bybit", [python_bin, "bybit.py"], ROOT / "bybit"))
    if enable_gate:
        insert_at = 3 if enable_bybit else 2
        specs.insert(insert_at, ProcSpec("gate", [python_bin, "gate.py"], ROOT / "gate"))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--enable-bybit", action="store_true", help="Enable Bybit collector")
    parser.add_argument("--enable-gate", action="store_true", help="Enable Gate collector")
